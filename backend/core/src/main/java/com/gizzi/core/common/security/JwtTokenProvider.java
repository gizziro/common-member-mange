...
public class JwtTokenProvider
{
	...
	private final JwtProperties 		jwtProperties;			// JWT 설정 프로퍼티 (기본값 역할)
	//----------------------------------------------------------------------------------------------------------------------
	...
	public String generateAccessToken(String userPk, String userId, String sessionId)
	{
		...
		//----------------------------------------------------------------------------------------------------------------------
		return Jwts.builder()
			.issuer(jwtProperties.getIssuer())		// 발급자 식별자 (admin / user)
			.subject(userPk)						// 사용자 PK (UUID)
			.claim("uid", userId)				// 로그인 ID
			.claim("sid", sessionId)				// 세션 PK (UUID)
			.issuedAt(Date.from(now))				// 발급 시각
			.expiration(Date.from(expiry))			// 만료 시각
