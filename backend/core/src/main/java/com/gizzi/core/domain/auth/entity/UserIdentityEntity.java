...
public class UserIdentityEntity
{
	...
	private String id;											// 식별자 PK (UUID)

	// [ 연관관계 ]
	//----------------------------------------------------------------------------------------------------------------------
	@ManyToOne(fetch = FetchType.LAZY)
	@JoinColumn(name = "user_id", nullable = false)
	private UserEntity user;									// 사용자 FK (지연 로딩)

	@ManyToOne(fetch = FetchType.LAZY)
	@JoinColumn(name = "provider_id", nullable = false)
	private AuthProviderEntity provider;						// 인증 제공자 FK (지연 로딩)

	// [ 소셜 계정 정보 ]
	//----------------------------------------------------------------------------------------------------------------------
	@Column(name = "provider_subject", nullable = false, length = 255)
	private String providerSubject;								// 제공자 측 사용자 고유 키 (sub, id 등)

	// [ 토큰 정보 ]
	//----------------------------------------------------------------------------------------------------------------------
	@Column(name = "access_token", length = 1024)
	private String accessToken;									// 제공자 Access Token (갱신용)

	@Column(name = "refresh_token", length = 1024)
	private String refreshToken;								// 제공자 Refresh Token (갱신용)

	@Column(name = "token_expires_at")
	private LocalDateTime tokenExpiresAt;						// 토큰 만료 일시
